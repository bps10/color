...
allows for fast square grid computation. I designed sompy for speed,
so attempting to read the code may not be very intuitive. If you're
trying to learn how SOMs work, I would suggest starting with Paras
Chopras SOMPython code:
...
If you do use this code for something, please let me know,
...
class SOM:
    ...
    def train(self, iterations=1000, train_vector=[[]]):
        """train_vector:
        [ FV0, FV1, FV2, ...] -> [ [...], [...], [...], ...]
        train vector may be a list, will be converted to a list of scipy
        arrays
        """
        for t in range(len(train_vector)):
            train_vector[t] = scipy.array(train_vector[t])
        time_constant = iterations / log(self.radius)
        delta_nodes = scipy.array([[[0
        ...
        for i in range(1, iterations + 1):
            ...
            learning_rate_decaying = self.learning_rate * exp(-1.0 * i /
                                                        time_constant)
    ...
    def best_match(self, target_FV):
        ...
        """
        loc = scipy.argmin((((self.nodes -
             target_FV) ** 2).sum(axis=2)) ** 0.5)
        r = 0
    ...
    try:
        ...
        for r in range(height):
            for c in range(width):
                img.putpixel((c, r),
                (int(color_som.nodes[r, c, 0]),
                int(color_som.nodes[r, c, 1]),
                int(color_som.nodes[r, c, 2])))
